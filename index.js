...
function normalizeHex (hex) {
  ...
  if (len === 3 || len === 4) {
    return Array.from(hex).reduce((prev, cur) => `${prev}${cur}${cur}`, '')
  }
  return hex
}

const toNum = (hex, start, end) => parseInt(hex.slice(start, end), 16)
    ...
    .walk(node => {
      ...
      if (nodes[0].value.startsWith('#')) {
        ...
        if (hex.length !== 3 && hex.length !== 6) {
          result.warn('Only 3 or 6 character hex allowed in rgba', {
            node: decl,
            word: nodes[0].value,
          })
          return
